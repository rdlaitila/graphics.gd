...
func (tr *Transformer) matchSelectorExpr(x, y *ast.SelectorExpr) bool {
	if xobj, ok := tr.wildcardObj(x.X); ok {
		...
		if o != nil {
			...
			return true && tr.info.Selections[x].Obj() == tr.info.Selections[y].Obj()
		}
	}
	xident, xok := x.X.(*ast.Ident)
	yident, yok := y.X.(*ast.Ident)
	if xok && yok && tr.info.Uses[yident] == nil && xident.Name == yident.Name && x.Sel.Name == y.Sel.Name {
		return true
	}
	return tr.matchExpr(x.X, y.X) && tr.info.Selections[x].Obj() == tr.info.Selections[y].Obj()
}
